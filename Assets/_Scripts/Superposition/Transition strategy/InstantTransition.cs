...
/// to a random position and rotation within a configurable radius around canonical.
...
public class InstantTransition : MonoBehaviour, IQuantumTransition
{
    ...
    private Quaternion _originRotation; // <-- NUEVO: Guardamos la rotación original
    private Vector3 _currentRealPosition;
    private SuperpositionController _controller;
    private bool _isGhostOnly;

    // Cambiamos List<Vector3> por List<QuantumObservation> para guardar toda la info (Pos y Rot)
    private readonly List<QuantumObservation> _observations = new();
    private readonly List<QuantumObservation> _remaining = new();
    ...
    private void Awake()
    {
        ...
        _originRotation = transform.rotation; // <-- Guardamos la rotación base
        ...
        if (observer != null)
        {
            ...
            foreach (var obs in observer.Observations)
                _observations.Add(obs); // <-- Ahora guardamos el objeto completo, no solo el Vector3
        }
        ...
        // Spawn a ghost at every position/rotation. Origin intentionally excluded here.
        foreach (var obs in _observations)
        {
            Vector3 targetPos = _originPosition + obs.offset;
            Quaternion targetRot = _originRotation * Quaternion.Euler(obs.rotationOffset); // <-- Calculamos rotación

            _ghosts[targetPos] = BuildGhost(targetPos, targetRot, transform.localScale);
        }
        ...
        if (_isGhostOnly)
        {
            ...
            if (!_ghosts.ContainsKey(_originPosition))
                _ghosts[_originPosition] = BuildGhost(_originPosition, _originRotation, transform.localScale);
        }
        else
        {
    ...
    {
        // OnValidate no puede leer fácilmente el ObserverTrigger si no está en Awake, 
        // pero podemos dejarlo vacío o con comprobaciones simples.
    }
    ...
    public SuperpositionState PickNextState(SuperpositionState current)
    {
        ...
        var obs = _remaining[index]; // <-- Sacamos la observación completa
        _remaining.RemoveAt(index);

        return new SuperpositionState
        {
            pos = _originPosition + obs.offset,
            rot = _originRotation * Quaternion.Euler(obs.rotationOffset), // <-- APLICAMOS LA ROTACIÓN AQUÍ
            scale = current.scale
        };
    }
    ...
    {
        _remaining.Clear();
        _remaining.AddRange(_observations);
    }
